...
export default function Dashboard() {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    if (!accessKey.trim()) { setAuthError('Access key is required.'); return; }
    ...
    return (
      <div className="gate">
        <form className="gate-form" onSubmit={handleLogin}>
          ...
          <p className="gate-sub">Enter your access key to unlock the dashboard</p>
          <input
            type="password"
            className="input"
          ...
          <button className="btn btn-primary" type="submit" disabled={checkingAuth}>
            {checkingAuth ? 'Checking...' : 'Unlock'}
          </button>
        </form>
      </div>
    );
